...
from compute_pipe.pipe import Pipe

# returns a deep copy of the pipe that is passed to it         
@_coconut_tco
def copy(pipe):
    raise _coconut_tail_call(Pipe, *pipe.steps)
...
#     fix_pipe_arg(check_validity, on_success, on_fail)
    ...
    def validator(data):
        ...
        if (check_validity(data)):
            ...
            if (on_success != None):
                raise _coconut_tail_call(on_success, data)
            else:
                return data
        else:
            if (on_fail != None):
                raise _coconut_tail_call(on_fail, data)
            return None
    return validator
